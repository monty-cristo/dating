...
window.onload = async function() {
  ...
  try {
    let url =
      "https://scrumserver.tenobe.org/scrum/api/ontgrendeling/wieIsVoorMijOntgrendeld.php?profielId=" +
    ...
    if (profielId == sessionStorage.getItem("user")) {
      ...
      document.getElementById("favoritesLink").style.display = "initial";
    } else {
      console.log("foo");
...
document.getElementById("lovecoinButton").onclick = async function(e) {
  ...
  try {
    //Unlock user
    let url =
      "https://scrumserver.tenobe.org/scrum/api/ontgrendeling/ontgrendel.php?profielId=" +
      sessionStorage.getItem("user");
...
function showData(profielData) {
  ...
  document
    ...
    .setAttribute(
      "alt",
      "foto van " + profielData.voornaam + " " + profielData.familienaam
    );
  document.getElementById("profielVan").innerText = profielData.nickname;
  document.getElementById("detailSexe").innerText = profielData.sexe;
